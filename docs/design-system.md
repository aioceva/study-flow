...
_Последна актуализация: 28 Май 2026_
...
export const NAV = {
  ...
  btnBorder: "#4A6FA5",   // border вариант (рядко използван)
...
```typescript
// Фон на content area
MODULE_COLORS  = { 1: "#F2F8FF", 2: "#F2FCF7", 3: "#FFF8FC", 4: "#F6F2FF" }

// Секции вътре в карта
MODULE_SURFACE = { 1: "#CBE8F8", 2: "#C4EAD0", 3: "#FFD4E8", 4: "#DDD0F4" }

// Прогрес бар сегменти
MODULE_PROGRESS = { 1: "#50B8DC", 2: "#58C090", 3: "#E89030", 4: "#8B7FCC" }

// Бутон Напред (бял текст върху него)
MODULE_BTN = { 1: "#1A3558", 2: "#0D3D20", 3: "#D4608A", 4: "#1E0B4A" }
```

**Правила:**
- Модул 3 фон (`#FFF8FC`) е розово-бял → тъмен текст `NAV.text` или `MODULE_BTN[3]` (#D4608A)
- Никога `#000000` — само `NAV.text` (#4A6FA5)
- `MODULE_PROGRESS[1]` (#50B8DC) = акцент за "Учи урока" бутон и Curiosity карта
...
### Цветът носи смисъл
- Синьо = Модул 1, Зелено = Модул 2, Розово = Модул 3, Лилаво = Модул 4
- `#4A6FA5` = навигация, структура (NAV елементи)
- `#50B8DC` = "Учи урока" акцент + Curiosity карта
...
### Curiosity карта (`/lesson/curiosity`) — Карта 0 и Карта 21

Специален екран, различен от lesson картите. Появява се два пъти:
- **Карта 0** — преди урока, веднага след `lesson/confirm`. Показва само въпроса.
- **Карта 21** — след последната карта на модул 4, преди `lesson/done`. Показва въпроса + отговора + конфети.

**Дизайн:**
- Фон: `#EBF6FC` (много светла версия на `MODULE_PROGRESS[1]`) — свързан визуално с "Учи урока" бутона
- Label горе: `🔍 ПРЕДИ ДА ЗАПОЧНЕШ` (Карта 0) / `💡 ОТГОВОРЪТ` (Карта 21) — `text-sm font-medium`, `NAV.textMuted`
- Фактът: `text-xl`, `NAV.textMuted` — неутрален, приземяващ
- Въпросът: `text-xl font-bold`, `MODULE_PROGRESS[1]` (#50B8DC) — акцент, провокативен
- Субтитъл (само Карта 0): "Ще научиш отговора след урока." — `text-sm`, `NAV.textMuted`
- Отговорът (само Карта 21): `text-base`, `NAV.text` + конфети анимация + "Браво! 🎉"
- Footer: primary бутон `NAV.btnSolid` — "Започни урока →" (Карта 0) / "→" към `lesson/done` (Карта 21)

**Данните** живеят в `adaptation.json`:
```json
{
  "curiosity": {
    "question_fact": "Киселината в стомаха ти е толкова силна, че разтваря метал.",
    "question_hook": "Защо тогава не те разтваря теб?",
    "answer": "Стомахът сменя лигавицата си на всеки 3-4 дни — по-бързо отколкото киселината може да го увреди."
  }
}
```

**Генерира се** от Claude заедно с адаптацията. Може да се редактира ръчно в GitHub.
...
- Горен блок: обобщение (последна активност, активни дни тази седмица, най-труден предмет)
- Среден блок: хронологичен списък сесии, групирани по дата
- Седмичен календар: зелена точка = всеки ден с учене (независимо от типа сесия)
- **Session badges** (в `SessionList.tsx`):
  - „Учене" — `#EBF4FF` / `#3B7DD8` — завършен урок (`type: "learn", status: "completed"`)
  - „Започнат урок" — `#FEF3C7` / `#92400E` — частично учене (`type: "learn", status: "partial"`)
  - „Преговор" — `#F3EEFF` / `#7B5EA7` — reinforcement quiz (`type: "reinforcement"`)
- Partial badge се **скрива** автоматично ако за същия урок/дата има completed
